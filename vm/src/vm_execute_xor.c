...
static int		xor_process(t_arg *targ, t_type_arg *type_arg)
{
	int num1;
	int num2;
...
int				deal_xor(t_arena *tarena, t_map *tmap,
							int pc_command, t_proc *tproc)
{
	t_execute_variable	var;
	int					ret;

	if ((var.targ = get_ret_targ(tmap, &ret,
					OP_XOR + 1, pc_command)) == NULL)
		return (ret);
	fill_var(&var, pc_command, tproc, tmap);
	if (get_first_second_arg(&var, 0) || get_first_second_arg(&var, 1))
	{
		t_arg_destroy(var.targ);
		return (ret);
	}
	get_third_arg(&var);
	var.type_arg.val_reg[3] = xor_process(var.targ, &(var.type_arg));
	...
	if (tarena->option & COMMANDS)
		show_commands_xor(var.tproc, var.targ, var.type_arg);
	t_arg_destroy(var.targ);
	return (ret);
}
