...
static int		fill_tarena_champions(t_arena *tarena, int argc,
										char *argv[], int color)
{
	int		idx_argv;
	int		cham_num;
	...
	ft_bzero(tarena->tcham, sizeof(t_champion*));
	tarena->num_cham = 0;
	while (idx_argv < argc)
	{
		if (idx_argv + 2 < argc &&
		...
		if (tarena->tcham[tarena->num_cham] == NULL)
			return (-1);
...
t_arena			*t_arena_new(int argc, char *argv[])
{
	...
	ft_bzero(tarena, sizeof(t_arena));
	fill_tarena_basic(tarena, argc, argv);
	if (fill_tarena_champions(tarena, argc, argv, 1) == -1)
	{
		t_arena_destroy(tarena);
		return (NULL);
	}
	if (tarena->num_cham > MAX_PLAYERS)
	{
		t_arena_destroy(tarena);
